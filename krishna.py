...
class PortScannerApp:
    def __init__(self, root):
        self.root = root    
        self.root.title("Serial Port Scanner & Connector")   
        ...
        scan_btn.grid(row=1, column=0, columnspan=2, pady=5, sticky=(tk.W, tk.E))
        ...
        self.connect_btn.grid(row=4, column=0, pady=5, sticky=(tk.W, tk.E), padx=2)
        
        # Disconnect button
        self.disconnect_btn = ttk.Button(main_frame, text="📥 Close Port", command=self.disconnect_port, state=tk.DISABLED)
        self.disconnect_btn.grid(row=4, column=1, pady=5, sticky=(tk.W, tk.E), padx=2)
        
        # Data display
        data_frame = ttk.LabelFrame(main_frame, text="Received Data", padding="5")
        data_frame.grid(row=5, column=0, columnspan=2, pady=10, sticky=(tk.W, tk.E, tk.N, tk.S))
        ...
        export_frame.grid(row=6, column=0, columnspan=2, pady=10, sticky=(tk.W, tk.E))
    ...
    def export_to_pdf(self):
        ...
        try:
            ...
            title = Paragraph("Serial Port Data Export", title_style)
            story.append(title)
            story.append(Spacer(1, 12))
            
            # Add metadata
            metadata_style = styles['Normal']
            story.append(Paragraph(f"<b>Exported:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", metadata_style))
            story.append(Paragraph(f"<b>Port:</b> {self.port_label.cget('text')}", metadata_style))
            story.append(Spacer(1, 12))
            ...
            # Skip header/metadata lines and find data rows
            data_start_idx = 0
            for idx, line in enumerate(lines):
            ...
            # Add data summary
            story.append(Paragraph(f"<b>Total Records:</b> {len(table_data) - 1} | <b>Columns:</b> {col_count}", metadata_style))
